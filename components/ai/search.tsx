  ...
  return (
    ...
    >
      <div className="px-3 py-2 flex-1">
        <p className="text-sm font-medium mb-2">问 AI</p>
        <p className="text-xs text-fd-muted-foreground">回答可能不准，请对照文档核对。</p>
      </div>
        ...
        return (
          ...
          >
            ...
            {call.state === 'output-error' || call.state === 'output-denied' ? (
              ...
              <p>{!call.output ? '搜索中…' : `${call.output.length} 条结果`}</p>
            )}
          </div>
        );
      })}
    ...
    >
      {messages.length === 0 ? (
        <div className="text-sm text-fd-muted-foreground/80 size-full flex flex-col items-center justify-center text-center gap-2">
          ...
          <p onClick={(e) => e.stopPropagation()}>在下方输入问题开始对话。</p>
        </div>
      ) : (
